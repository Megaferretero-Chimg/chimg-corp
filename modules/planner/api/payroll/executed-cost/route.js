...
import { isEmployeeDismissedInMonth } from "@/modules/company/submodules/people/lib/employees";
import {
...
export async function GET(request) {
  ...
  try {
    ...
    const supplementaryMultiplier = SUPPLEMENTARY_PAY_MULTIPLIER;
    const extraordinaryMultiplier = EXTRAORDINARY_PAY_MULTIPLIER;
    const rows = filteredRows.map((row) => {
      const employeeId = row.employee?.toString?.() || "";
      const employee = employeesById.get(employeeId);
      const salary = Number(employee?.salary) || 0;
      const hourlyRate = Number(row.hourlyRate) || calculatePayrollHourlyRate(salary, hourlyDivisor);
      ...
      const normalCost = shouldUseClosedBase && hasClosedSalaryBase && Number.isFinite(rowSalaryBase)
        ? rowSalaryBase
        : salary;
      const supplementaryCost = supplementaryHours * calculatePayrollAdditionalRate(hourlyRate, supplementaryMultiplier);
      const extraordinaryCost = extraordinaryHours * calculatePayrollAdditionalRate(hourlyRate, extraordinaryMultiplier);
