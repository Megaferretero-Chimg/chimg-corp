...
export default function EmployeeMonthlySummaryView({
  ...
  const isFilterDisabled = isCatalogLoading || isLoadingReport || isSavingPayment;
  ...
  useEffect(() => {
    ...
    async function loadPayment() {
      try {
        ...
        const response = await fetch(`/api/planner/payroll/payments?${params.toString()}`);
        const data = await response.json();
  ...
  async function savePayment(event) {
    ...
    try {
      ...
      const response = await fetch("/api/planner/payroll/payments", {
        method: "POST",
        headers: {
  ...
  return (
    <section className={styles.panel}>
      <div className={styles.controls}>
        <label>
          <span>Empleado</span>
          <select value={employeeId} onChange={(event) => handleEmployeeChange(event.target.value)} disabled={isFilterDisabled}>
