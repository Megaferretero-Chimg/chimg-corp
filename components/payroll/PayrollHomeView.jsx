  ...
  const load = useCallback((nextFilters) => {
    startTransition(async () => {
      ...
      const [plannedResult, executedResult, closureResult] = await Promise.allSettled([
        fetchJson(`/api/payroll/planned-cost?${params.toString()}`),
        fetchJson(`/api/payroll/executed-cost?${params.toString()}`),
        fetchJson(`/api/attendance/monthly-closure?month=${encodeURIComponent(nextFilters.month)}`),
      ]);
  ...
  return (
    <section className={styles.panel}>
      <div className={styles.filterBar}>
        ...
        <label>
          <span>Área</span>
          <select value={filters.areaCode} onChange={(event) => updateFilter("areaCode", event.target.value)}>
            <option value="">Todas</option>
